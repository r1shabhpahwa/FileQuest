...
#define CONNECT_SERVER 'S'
#define CONNECT_MIRROR 'M'
...
// Determine which server to connect to: the primary server or the mirror server.
char loadBalance(int clientNo);
...
// Function to redirect client to mirror
int redirectToMirror(int clientSockfd);
...
int main(int argc, char *argv[])
{
	...
	{	
		...
		char connectType = loadBalance(client_no);
		
		// Connect 
		if(connectType == CONNECT_SERVER){
		...
		else if(connectType == CONNECT_MIRROR) {
			csd = accept(sd, (struct sockaddr *) NULL, NULL);
			printf("[~] Server Full: Redirecting client %d\n", client_no);
			
			// Fork a child process to redirect client
			if (!fork()){	
				redirectToMirror(csd);
				close(csd);
				exit(0);
			}
			waitpid(0, &status, WNOHANG);
...
void processClient(int client_sockfd)
{
	...
	sendTextResponse(client_sockfd, "Send commands now");
...
char loadBalance(int clientNo) {
    ...
    } else if (clientNo >= 5 && clientNo <= 8) {
        return CONNECT_MIRROR;
    } else {
        static char result = CONNECT_MIRROR;  
        result = (result == CONNECT_SERVER) ? CONNECT_MIRROR : CONNECT_SERVER;  
        return result;
    }
}

int redirectToMirror(int client_sockfd){
	AddressInfo address_info;
	
	// Send response type mirror struct
	long responseType = RESPONSE_STRUCT;
	send(client_sockfd, &responseType, sizeof(responseType), 0);
	
	// Send mirror IP and Port
	strcpy(address_info.ip_address, MIRROR_IP);
	address_info.port_number = MIRROR_PORT;
	send(client_sockfd, &address_info, sizeof(AddressInfo), 0);
	printf("[+] Client %d redirected\n", client_no);
